...
puts {
  ...
  <div class="row">
    <div class="col s12">
      ...
      Moment Geduld bis die notwendigen Daten auf die Zentrale &uuml;bertragen worden sind.
      <p>
...
$(document).ready(function() {
  ...
  $("#registerbutton").click(function() {
    if (!$("#registerform").valid()) { $("#error_modal").openModal(); } else {
      // check mail-address
      $.post("https://www.meine-homematic.de/script/checkMailAddress.php", { mail: $("#MAIL").val() }).done(function(data) {
        if (jQuery.parseJSON(data).result == 0) { // address exists
          $("#validation_error").html('Die von Ihnen angegebene EMail-Adresse ist leider schon registriert.<br><br>Sofern Sie eine weitere EMail-Adresse besitzen, geben Sie bitte diese an. Sollte dies nicht der Fall sein, k&ouml;nnen Sie sich bei diversen EMail-Anbietern (z.B. gmail, gmx) kostenfrei eine weitere Adresse registrieren.');
