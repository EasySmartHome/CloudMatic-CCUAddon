...
if {$dienstzbx == 1} {
	if {[catch {exec /bin/sh /etc/config/addons/mh/dienstauszbx.sh } result]} {
	...
	puts "Der CloudMatic monitoring Dienst wurde beendet."
}
if {$dienstzbx == 0} {
	puts "Der CloudMatic monitoring Dienst ist bereits deaktiviert."
}
