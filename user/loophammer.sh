...
# Hammering-Schutz:
# ==================
# Das Skript kontrolliert, ob das aktuelle Datum nach dem Datum liegt, bis zu dem der Premium Zugang g�ltig ist.
...
VERSION=`cat /VERSION |grep PRODUCT |awk -F"=" '{ print $2 }'|awk -F"_" '{print $1}'`
  ...
  # 
  nowdate=$(date +%s)
  # Enddatum wird individuell gesetzt, in Abh�ngigkeit der Schl�ssel - Laufzeit
  enddate=$(date -d "010123592010" +%s)
  if [ $enddate -ge $nowdate ] 
  then  
     #
     # Aktuelles Datum ist vor dem Enddatum. VPN starten, falls es nicht l�uft
     #
    ...
    if [ -n "`pidof $Processname`" ] ; then  
      ...
      /bin/busybox logger -t homematic -p user.info "Diest dient zum Schutz Ihrer HomeMatic, um permanente, ung�ltige Verbindungsversuche alle 10 Sekunden zu unterbinden."
  ...
  #Wenn Dienst l�uft beenden, falls Dienst = 0
    if [ $dienst -lt 1 ] ; then
		Processname=openvpn
		if [ -n "`pidof $Processname`" ] ; then  
